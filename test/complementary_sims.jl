...
@testset "Complimentary simulations" begin
    ...
    @time (x⃗1, Ψ1), (x⃗2, Ψ2) = simulate_galaxies(nbar, L, pk; nmesh=n_sim, bias, f=true, extra_phases=π)
    #@time x⃗1, Ψ1 = simulate_galaxies(nbar, L, pk; nmesh=n_sim, bias, f=true)
    #@time x⃗2, Ψ2 = simulate_galaxies(nbar, L, pk; nmesh=n_sim, bias, f=true)
    ...
    plot(k1, pk1[:,1] ./ (bias^2 .* pk.(k1)), "-")
    plot(k2, pk2[:,1] ./ (bias^2 .* pk.(k1)), "-")
    plot(k2, (pk1 .+ pk2)[:,1] ./ 2 ./ (bias^2 .* pk.(k1)), "k:")
    xlim(0, 0.25)
    ylim(0.6, 1.4)
