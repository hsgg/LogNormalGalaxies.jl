...
Inserting \cref{eq:delta_deltaG} into
\ba
...
% \xi(\vr)
% &=
% \< \delta(\vx) \,\delta(\vx+\vr) \>
% \\
% &=
% \< \left[e^{G(\vx) - \frac12\<G^2\>}-1\right]
% \left[e^{G(\vx+\vr) - \frac12\<G^2\>}-1\right]\>
% \\
% &=
% \< e^{G(\vx) + G(\vx+\vr)} \> e^{- \<G^2\>}
% - 1
% \\
% &=
% e^{\frac12\<[G(\vx) + G(\vx+\vr)]^2\>} e^{- \<G^2\>}
% - 1
% \\
% &=
% e^{\<G(\vx)G(\vx+\vr)\>}
...
\mu_3
&=
e^{3\mu + \frac32\sigma^2}
\left[e^{\sigma^2} - 1\right]^2
\left[e^{\sigma^2} + 2\right].
\ea
The generalization is
\ba
\mu_3^{ijk}
&=
e^{\mu_i + \mu_j + \mu_k + \frac12\left[\Sigma_{ii} + \Sigma_{jj} + \Sigma_{kk}\right]}
\vs&\quad\times
\left[e^{\Sigma_{ij} + \Sigma_{jk} + \Sigma_{ki}}
  - e^{\Sigma_{ij}}
  - e^{\Sigma_{jk}}
  - e^{\Sigma_{ki}}
+ 2\right].
...
To get the bispectrum, transform to Fourier space.
In \cref{eq:ln_3pt} the terms with two 2-point functions can be converted to
Fourier space in the following way. For example,
\ba
B_{12,23}
&=
(2\pi)^3 \delta^D(\vk_{123})
\int\dd^3s_{12}
\,e^{-i\vk_1\cdot\vvs_{12}}
\,\xi(\vvs_{12})
\vs&\quad\times
\int\dd^3s_{23}
\,e^{i\vk_3\cdot\vvs_{23}}
\,\xi(\vvs_{23})
\\
&=
(2\pi)^3 \delta^D(\vk_{123})
...
\,P(\vk_3)\,.
\ea
The term with three 2-point functions involves a convolution,
\ba
B_{12,23,31}
&=
\int\dd^3r_{1}
...
\,e^{-i\vk_3\cdot\vr_{3}}
\vs&\quad\times
\int\frac{\dd^3q_1}{(2\pi)^3}\,e^{i\vq_1\cdot\vvs_{12}}\,P(\vq_1)
\vs&\quad\times
\int\frac{\dd^3q_2}{(2\pi)^3}\,e^{i\vq_2\cdot\vvs_{23}}\,P(\vq_2)
\vs&\quad\times
\int\frac{\dd^3q_3}{(2\pi)^3}\,e^{i\vq_3\cdot\vvs_{31}}\,P(\vq_3)
\\
&=
\int\frac{\dd^3q_1}{(2\pi)^3}
\int\frac{\dd^3q_2}{(2\pi)^3}
\int\frac{\dd^3q_3}{(2\pi)^3}
\,P(\vq_1)
\,P(\vq_2)
\,P(\vq_3)
\vs&\quad\times
(2\pi)^3 \delta^D(\vk_1 + \vq_3 - \vq_1)
\vs&\quad\times
(2\pi)^3 \delta^D(\vk_2 + \vq_1 - \vq_2)
\vs&\quad\times
(2\pi)^3 \delta^D(\vk_3 + \vq_2 - \vq_3)
\\
&=
(2\pi)^3 \delta^D(\vk_1 + \vk_2 + \vk_3)
\vs&\quad\times
\int\frac{\dd^3q_3}{(2\pi)^3}
\,P(\vk_1 + \vk_2 + \vq_3)
\,P(\vk_2 + \vq_3)
\,P(\vq_3)
\ea
The bispectrum multipoles are given by first putting the multipole on the first
delta, then binning in $k_i$ shells. We get
\begin{widetext}
\ba
B^\mathrm{meas}_\ell(k_1,k_2,k_3)
&=
\frac{1}{(2\pi)^3N^{123}_\mathrm{tri}}
\int_{k_1}\dd^3k_1'
\int_{k_2}\dd^3k_2'
\int_{k_3}\dd^3k_3'
%\vs&\quad\times
...
\int\dd^3x
\,e^{-i\vk_1'\cdot\vx}
\,\mathcal{L}_\ell(\khat_1'\cdot\xhat)
\int\frac{\dd^3q_1'}{(2\pi)^3}
\,e^{i\vq_1'\cdot\vx}
\<\delta(\vq_1')
\,\delta(\vk_2')
...
\begin{figure*}
  \centering
  \incgraph{../figs/pkest_redshiftspace_nmesh256_simCIC_estNGP_velo1_fxshift0.0_p3_pkest_rdiff.pdf}
  \incgraph{../figs/pkest_redshiftspace_nmesh256_simCIC_estNGP_velo1_fxshift0.5_p3_pkest_rdiff.pdf}
  \incgraph{../figs/pkest_redshiftspace_nmesh512_simCIC_estNGP_velo1_fxshift0.0_p3_pkest_rdiff.pdf}
  \incgraph{../figs/pkest_redshiftspace_nmesh512_simCIC_estNGP_velo1_fxshift0.5_p3_pkest_rdiff.pdf}
...
\begin{figure*}
  ...
  \incgraph{../figs/pkest_redshiftspace_nmesh512_simCIC_estNGP_velo2_fxshift0.0_p3_pkest_rdiff.pdf}
  \incgraph{../figs/pkest_redshiftspace_nmesh512_simCIC_estNGP_velo2_fxshift0.5_p3_pkest_rdiff.pdf}
...
\begin{figure*}
  ...
  \incgraph{../figs/pkest_redshiftspace_nmesh512_simCIC_estNGP_velo3_fxshift0.0_p3_pkest_rdiff.pdf}
  \incgraph{../figs/pkest_redshiftspace_nmesh512_simCIC_estNGP_velo3_fxshift0.5_p3_pkest_rdiff.pdf}
...
\begin{figure*}
  ...
  \incgraph{../figs/pkest_redshiftspace_nmesh512_simCIC_estNGP_velo4_fxshift0.0_p3_pkest_rdiff.pdf}
  \incgraph{../figs/pkest_redshiftspace_nmesh512_simCIC_estNGP_velo4_fxshift0.5_p3_pkest_rdiff.pdf}
