...
Several things to consider here. First, it appears that in
\citet{Agrawal+:2017JCAP...10..003A} they only consider power spectra with
small amplitude. In such cases, the log-normal nature is nearly irrelevant,
because $\ln(1+\delta)\simeq\delta$. Also, $\xi_G(0)$ is well-defined. Is
$P_G(0)$ well-defined?

Numerically, I find that the variation of $\sigma_G^2$ from one simulation is
within machine precision. This seems off to me. Perhaps we can estimate what
...
Finally, we draw galaxies from the real-space density contrast. The number of
...
to each galaxy within a grid cell.
